...
import type { ICheckoutRequest, IOrderServiceResponse, IOrderResponse } from "../interfaces/orderInterfaces";
...
export const checkout = async (userId: number, data: ICheckoutRequest): Promise<IOrderServiceResponse<IOrderResponse>> => {
  try {
    ...
    for (const item of cart.items) {
      ...
      const product = await ProductRepository.findProductById(item.product_id);
      if (product) await ProductRepository.updateProduct(product.id, { stock: product.stock - item.quantity });
    }

    await CartRepository.markCartAsConverted(cart.id);
    logger.info(` Order ${order.id} created successfully`);

    const finalOrder = await OrderRepository.findOrderById(order.id);
    return { success: true, message: ORDER_MESSAGES.ORDER.CHECKOUT_SUCCESS, data: mapToResponse(finalOrder!), timestamp: new Date().toISOString() };
  } catch (error) {
...
export const getUserOrders = async (userId: number): Promise<IOrderServiceResponse<IOrderResponse[]>> => {
  try {
    ...
    return { success: true, message: ORDER_MESSAGES.ORDER.FETCH_SUCCESS, data: orders.map(mapToResponse), timestamp: new Date().toISOString() };
  } catch (error) {
    logError(error as Error, { endpoint: "OrderService.getUserOrders", body: { userId } });
...
export const getOrderById = async (userId: number, orderId: number): Promise<IOrderServiceResponse<IOrderResponse>> => {
  try {
    ...
    if (!order || order.user_id !== userId) {
      return { success: false, message: order ? ORDER_MESSAGES.ORDER.UNAUTHORIZED : ORDER_MESSAGES.ORDER.NOT_FOUND, timestamp: new Date().toISOString() };
    }
    return { success: true, message: ORDER_MESSAGES.ORDER.FETCH_SUCCESS, data: mapToResponse(order), timestamp: new Date().toISOString() };
  } catch (error) {
    logError(error as Error, { endpoint: "OrderService.getOrderById", body: { userId, orderId } });
    return { success: false, message: ORDER_MESSAGES.ORDER.FETCH_FAILED, timestamp: new Date().toISOString() };
  }
};
...
export const cancelOrder = async (userId: number, orderId: number): Promise<IOrderServiceResponse> => {
  try {
    ...
    if (order.status !== OrderStatus.PENDING && order.status !== OrderStatus.CONFIRMED) {
      return { success: false, message: ORDER_MESSAGES.ORDER.NOT_CANCELLABLE, timestamp: new Date().toISOString() };
    }
    ...
    return { success: true, message: ORDER_MESSAGES.ORDER.CANCEL_SUCCESS, timestamp: new Date().toISOString() };
  } catch (error) {
    logError(error as Error, { endpoint: "OrderService.cancelOrder", body: { userId, orderId } });
    return { success: false, message: ORDER_MESSAGES.ORDER.CANCEL_FAILED, timestamp: new Date().toISOString() };
  }
};